    ...
    for dir in &dirs {
        ...
        for file in files {
            ...
            for r in &mut records {
                if r.hour_start.is_empty() { r.hour_start = bucket.clone(); }
            }
...
fn parse_transcript_line(v: &Value, source: &str) -> Option<UsageRecord> {
    ...
    let output = usage.get("candidatesTokenCount")
        ...
        .and_then(|x| x.as_u64()).unwrap_or(0);
    let cached = usage.get("cachedContentTokenCount").and_then(|x| x.as_u64()).unwrap_or(0);
    let total = usage.get("totalTokenCount").and_then(|x| x.as_u64()).unwrap_or(input + output);
