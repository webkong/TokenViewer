...
pub fn parse(home_dir: &Path, cursor_data: Option<&str>) -> Result<(Vec<UsageRecord>, String), Box<dyn std::error::Error>> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok((id, model_cfg, created_at, total, input, output, acc_total, acc_input, acc_output))
    })?;

    for row in rows.flatten() {
        let (id, model_cfg, created_at, total, input, output, acc_total, acc_input, acc_output) = row;
