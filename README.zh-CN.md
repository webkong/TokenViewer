...
一款用 **SwiftUI + Rust** 构建的原生 macOS 菜单栏应用，自动收集 **25 个 AI 编程工具**的 Token 用量和费用，在本地聚合后呈现在精美的仪表盘中。无需云端账号、无需 Node.js、无需浏览器——只是一个小巧的原生应用。
...
| **WorkBuddy** | `~/.antigravity_cockpit/workbuddy_accounts/*.json` | 本地配额快照读取 |
...
TokenViewer 可以显示 9 个工具的实时配额用量和重置倒计时：
...
| **WorkBuddy** | Pro / Free / Enterprise | `codebuddy.cn` 计费接口（`workbuddy-desktop.info`） |
...
```
AI 工具日志  →  25 个 Rust 解析器  →  SQLite (~/.tokenviewer/data.db)
                                              ↓
...
- **Rust 核心** (`core/`) — 25 个解析器、SQLite 存储、定价引擎，编译为 `libtokenviewer_core.a`
